...
try {
    ...
    # Log payload info
    Log-Event -EventType "payload_info" -Data @{
        image_base = "0x{0:X}" -f $imageBase
        entry_point_rva = "0x{0:X}" -f $entryPointRVA
...
Log-Event -EventType "section_created" -Data @{
    section_name = $sectionName
    section_handle = $hSection.ToInt64()
    size = $secSize
}
...
# Log local mapping
Log-Event -EventType "local_mapping" -Data @{
    base_address = "0x{0:X}" -f $localBase.ToInt64()
    size = [long]$viewSize
}
...
Log-Event -EventType "process_created" -Data @{
    process_id = $pi.dwProcessId
    thread_id = $pi.dwThreadId
    process_handle = $pi.hProcess.ToInt64()
    thread_handle = $pi.hThread.ToInt64()
}
...
# Log remote image base
Log-Event -EventType "remote_image_base" -Data @{
    base_address = "0x{0:X}" -f $remoteImageBase.ToInt64()
}
...
# Log remote mapping
Log-Event -EventType "remote_mapping" -Data @{
    base_address = "0x{0:X}" -f $remoteBase.ToInt64()
    size = [long]$viewSize
}
...
# Log APC queuing
Log-Event -EventType "apc_queued" -Data @{
    target_address = "0x{0:X}" -f $apcRoutine.ToInt64()
    thread_id = $pi.dwThreadId
}
...
# Log thread resumption
Log-Event -EventType "thread_resumed" -Data @{
    thread_id = $pi.dwThreadId
}
